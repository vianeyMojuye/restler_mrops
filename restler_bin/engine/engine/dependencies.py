...
def set_equivalence_post_codes(resp_code , resp_body, resp_id, method="POST"):
        ...
        """
         if resp_code is not None:
            if method == "POST":
                equivalence_post_codes.append({'status_code' : resp_code , 'resp_body' : resp_body , 'resp_id' : resp_id })  
            elif method == "PUT":
                equivalence_put_codes.append({'status_code' : resp_code , 'resp_body' : resp_body , 'resp_id' : resp_id })  

            elif method == "DELETE":
                equivalence_delete_codes.append({'status_code' : resp_code , 'resp_body' : resp_body , 'resp_id' : resp_id })  

            elif method == "GET":
                equivalence_get_codes.append({'status_code' : resp_code , 'resp_body' : resp_body , 'resp_id' : resp_id })  


#  Python Program  to compare status_code basant resp_id
def compare_status_codes(L1, L2,method):
    results = []
    print(L1,L2)
    for item in L2:
...
def compare_for_equality_mrop(L1, L2,method):
    ...
    r = all(
        e1['status_code'] == e2['status_code'] and e1['resp_id'] != e2['resp_id'] and e1['resp_body'] != e2['resp_body']
